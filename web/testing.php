<?
	...
	$base_url = "http://www.openecology.in/indianorthobib/?" . $url;

	$keyword_url = "http://www.openecology.in/indianorthobib/?" . $kw_url;
...
.showmore_text { font-size:9px; font-style:italic; }
...
<script src="http://ajax.googleapis.com/ajax/libs/jquery/1.5/jquery.min.js"></script>
   <script>
     $(document).ready(function(){
	  ...
	  $('#show_' + id).toggle();
	  var text = $('#_' + id).html();
          $('#_' + id).html( text == "..show all" ? "..show less" : "..show all");
       });
	...
	<div class='paginator'>
		...
		<?
		   ...
		   if($search_results) {
		     ...
		     foreach ($search_results as $sri) {
			     ...
			     if( count($sri['ED']) == '1' ) {
			       ...
			       for($i=0; $i < count($sri['ED']); $i++) {
		             ...
		             $keywords = '';
			     if( count($sri['KW']) == '1' ) {
			     	 $keywords = "<a href='" . $keyword_url . "&bib_search_list=KW&bib_search_word=" . strtolower($sri['KW'][$i]) . "'>" . ucfirst(strtolower($sri['KW'][$i])) . "</a>, ";
 
			     } else if( count($sri['KW']) > '1' ) {
                                 //print_r($sri['KW']);
                                 $iter_count = count($sri['KW']);
                             
			        for($i=0; $i < $iter_count; $i++) {
                                  if($i == '5' ) {
					$keywords .= "<span class='showmore' id='show_" . $obj_id . "'>";
				   } 

			     	   $keywords .= "<a href='" . $keyword_url . "&bib_search_list=KW&bib_search_word=" . strtolower($sri['KW'][$i]) . "'>" . ucfirst(strtolower($sri['KW'][$i])) . "</a>, ";

			          if( $i == ($iter_count - 1 ) && $i >= '5' ) {
				     $keywords .= "</span><a href='#x' id='" . $obj_id . "' class='showmore_text'>..show all</a>";
				  }
			 ...
			 echo "<div class='bibkey'>Keywords: <span class='bibkey_text'>" . $keywords . "</span></div>";
			 echo "</li>";
